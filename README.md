...
Ptu 是一个 Windows / macOS 桌面工具，用于下载抖音图文、视频和实况照片素材，并把图文/实况素材合成为干净的 9:16 竖屏 MP4。

当前版本：`v1.5.0`

下载地址：[GitHub Releases](https://github.com/Fairc11/Ptu-Fairc11/releases)

Mac 版测试包：[Ptu V1.5 Mac 版](https://github.com/Fairc11/Ptu-Fairc11/releases/tag/v1.5.0-mac.1)
...
1. 打开 [Ptu V1.5 Mac 版](https://github.com/Fairc11/Ptu-Fairc11/releases/tag/v1.5.0-mac.1)
2. 下载 `Ptu-v1.5.0-mac-arm64.dmg`
3. 打开 DMG，把 `Ptu.app` 拖到 `Applications`
4. 首次打开如果被 macOS 拦截，在“系统设置 -> 隐私与安全性”里允许打开
5. 启动 Ptu 后用系统浏览器打开抖音，手动复制分享链接，再回到 Ptu 粘贴抓取

Mac 版 DMG 已内置 FFmpeg、FFprobe 和 Playwright Chromium，不需要用户安装 Python、FFmpeg、Playwright 或 Chromium。

当前 Mac 版未做 Apple Developer ID 签名和公证，首次打开会触发 macOS 安全提示。
...
- Windows 版使用 WebView2 / WinForms 内嵌右侧抖音预览 Dock。
- Mac 版使用 pywebview Cocoa 桌面壳，不启用 Windows WebView2 预览。
- Mac 版打开抖音时会调用系统浏览器，用户手动复制分享链接后回到 Ptu 粘贴抓取。
- Windows 版分发 `.exe` 安装器；Mac 版分发 `.dmg`。
- 两个平台共用下载、任务库、诊断包脱敏和视频合成逻辑。
...
build_macos.sh               macOS .app / DMG 打包脚本
installer.iss                Inno Setup 安装器配置
```

## 更新日志

### v1.5.0-mac.1 — 2026-07-09

**新增**

- 新增 macOS arm64 DMG 测试包：`Ptu-v1.5.0-mac-arm64.dmg`。
- 新增 macOS PyInstaller 打包配置和 `build_macos.sh`。
- Mac 桌面壳使用 pywebview Cocoa，保留 Ptu 主界面和后端能力。
- Mac 版内置 FFmpeg、FFprobe 和 Playwright Chromium。

**调整**

- Mac 版不启用 Windows WebView2 / WinForms 内嵌抖音预览，改为系统浏览器打开抖音。
...
- 封包态带音乐合成视频：通过
- 封包态诊断包创建：通过
- `hdiutil verify dist/Ptu-v1.5.0-mac-arm64.dmg`：通过
- `codesign --verify --deep --strict --verbose=2 dist/Ptu.app`：通过

**已知限制**

- 当前 DMG 未签名、未公证，首次打开需要按 macOS 安全提示手动允许。
- Mac 版暂不提供 Windows 版右侧内嵌抖音 WebView2 预览。
